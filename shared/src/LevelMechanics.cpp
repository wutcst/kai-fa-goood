...
void configureSawTravelBounds(const GameMap& map, std::vector<SawTrap>& saws) {
    ...
    for (SawTrap& saw : saws) {
        ...
        const float maxUpAmplitude = std::max(0.0f, saw.anchorY - (ceilingBottom + kTravelMargin));
        const float maxDownAmplitude = std::max(0.0f, (floorTop - kTravelMargin) - (saw.anchorY + saw.height));
